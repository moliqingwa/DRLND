...
from model_pixels import PixelDQNNetwork
...
class Agent(object):
    def __init__(self, in_channels, action_size,
        ...
        :param in_channels: channel dimension of each state
        ...
        self._model_local = PixelDQNNetwork(self.in_channels, self.action_size).to(device)
        self._model_target = PixelDQNNetwork(self.in_channels, self.action_size).to(device)
...
def dqn(env, brain_name, agent, n_epochs,
    ...
    for i_epoch in range(n_epochs):
        ...
        while True:
            ...
            next_state = env_info.visual_observations[0].transpose(0, 3, 1, 2)
...
if __name__ == "__main__":
    ...
    state = state.squeeze().transpose(2, 0, 1)  # convert to (c x h x w)
    print('State shape look like:', state.shape)

    env_info = env.reset(train_mode=True)[brain_name]

    agent = Agent(in_channels=state.shape[0],
                  action_size=brain.vector_action_space_size,
                  batch_size=32,
                  learning_rate=1e-4,
                  gamma=0.95,
    ...
    smoothed_scores, scores, losses = dqn(env=env, brain_name=brain_name,
                                          agent=agent,
                                          n_epochs=500,
                                          checkpoint_file="visual_model.pt",
                                          epsilon=1.0,
                                          epsilon_decay=0.99,
                                          epsilon_min=0.005)
